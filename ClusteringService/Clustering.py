...
class ClusteringService:
    ...
    @staticmethod
    def execute_clustering(self, F: pd.DataFrame, K: int) -> dict:
        results = {}

        results['k'] = K
        results['kmedoids'] = self.run_kmedoids(F, K)
        results['silhouette'] = self.run_calculate_silhouette_valuekmedoids(F, results['kmedoids']['labels'])
    ...
    def run_kmedoids(F: pd.DataFrame, K: int, method: str = 'pam', init: str = 'k-medoids++', random_state: int = 42) -> dict:
        ...
        """
        kmedoids = KMedoids(n_clusters=K, method=method, init=init, random_state=random_state).fit_predict(F)
        results = {
